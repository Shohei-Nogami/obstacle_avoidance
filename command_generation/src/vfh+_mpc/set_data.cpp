...
void VFH_MPC::set_mv_obstacle_data(const std::vector<cv::Point2f>& pts, const float vx0, const float vy0)
{
...
void VFH_MPC::end_set_mv_obstacle_data(void)
{
	mv_obsts.resize(mv_obsts_size);
}
void VFH_MPC::clear_mv_obstacle_data(void)
{
	mv_obsts.clear();
...
void VFH_MPC::move_obstacle_data(float& time)
{
...
void VFH_MPC::clear_move_data(void)
{
	for (int k = 0; k < mv_obsts.size(); k++) {
		mv_obsts[k].mvx = 0;
		mv_obsts[k].mvy = 0;
