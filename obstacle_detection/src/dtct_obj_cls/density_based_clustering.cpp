...
void detect_objects::density_based_clustering(cv::Mat& image)
{
	...
	obstacle_detection::cluster_element Q_p;
	int **cluster_index;
	...
	//std::vector<obstacle_detection::point2i> task_objects;
	obstacle_detection::cluster_element task_objects;
	...
	//cv::Point2i temp;
	obstacle_detection::cluster_point temp;
	...
	for(int h=0+search_range;h<height/ksize-search_range;h++){
		...
		for(int w=0+search_range;w<width/ksize-search_range;w++){
			...
			for(int i=0;i<task_objects.pt.size();i++){
				...
				obstacle_detection::cluster_element task_pt;
				int pt_n=0;
				...
				if(min_pn<((int)task_pt.pt.size()+pt_n))
				{
					...
					obstacle_detection::cluster_point pt_temp;
...
{
	obstacle_detection::cluster pub_Q;
